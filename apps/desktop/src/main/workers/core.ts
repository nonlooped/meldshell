...
if (parentPort === undefined) {
  throw new Error("The MeldShell core must run as an Electron utility process.")
}

const databasePath = process.env.MELDSHELL_DATABASE_PATH

if (databasePath === undefined) {
  throw new Error("MELDSHELL_DATABASE_PATH was not supplied to the core process.")
}
...
const exposeCoreRead = <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, CoreError, R> =>
  effect.pipe(
...
const CoreHandlersLive = CoreRpcs.toLayer({
  ...
  ListThreads: (input) => exposeCoreRead(listThreads(input)),
...
const makeElectronProtocol = RpcServer.Protocol.make((writeRequest) =>
  Effect.gen(function* () {
    ...
    return {
      ...
      end: (clientId: number) => Effect.sync(() => clientIds.delete(clientId)).pipe(Effect.asVoid),
