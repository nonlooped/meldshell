...
it.effect(
  "windows retain boundary turns; forward catch-up is ordered, bounded and thread scoped",
  () =>
    Effect.gen(function* () {
      ...
      const recent = yield* getTranscript({ threadId: "thread", limit: 2 })
      assert.deepEqual(
        recent.events.map((event) => event.sequence),
        [3, 4, 5, 6, 7, 8],
      )
      assert.equal(recent.nextCursor, 3)
      const old = yield* getTranscript({ threadId: "thread", beforeSequence: 3, limit: 2 })
      assert.deepEqual(
        old.events.map((event) => event.sequence),
        [1, 2],
      )
      assert.equal(old.nextCursor, null)
      const forward = yield* getTranscript({ threadId: "thread", afterSequence: 3, limit: 2 })
      assert.deepEqual(
        forward.events.map((event) => event.sequence),
        [4, 5],
      )
      assert.equal(forward.nextCursor, 5)
      const tail = yield* getTranscript({ threadId: "thread", afterSequence: 7, limit: 2 })
      assert.deepEqual(
